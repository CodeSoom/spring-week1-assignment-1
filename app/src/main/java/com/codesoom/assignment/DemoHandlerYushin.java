...
import com.codesoom.assignment.config.Regex;
...
public class DemoHandlerYushin implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if (isTasksRequest(pathSegments)) {
            try {
                HttpResponse httpResponse = fetchHttpResponse(exchange);
    ...
    private int getTaskPathVariable(String[] pathSegments) {
        if (!Regex.isOnlyNumber(pathSegments[2])) {
